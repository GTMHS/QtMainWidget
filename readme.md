...
* 现有的这个技术需要改进
* 需求：在1秒钟之内完成样本的检测识别和判断。并做出动作。
* 准确度要求：识别正确率达到99.99%。

### 功能设计
1. 相机控制：控制相机的开关、拉流方式，触发方式。光电开关控制相机的触发，相机将数据流传回至电脑端。

2. 识别控制：控制识别的开始和关闭。识别结果的返回。
3. 外设控制：控制回收装置回收残次图书。

### 开发文档
1. 软件启动初始化：初始化工作包括读取配置文件、软件界面显示空间的设置等。设置相机，设置了按钮able or disable，设置了列占比。读取了配置文件，配置文件中的串口号，配置文件中的k、b、s、N，位置信息，初始化detector。
做宣传用的样本和文字都在`init`中。

2. `testRun`方法为本地测试用的方法，会读取本地文件，而非连接摄像头进行读取相机流信息。本地文件的保存路径为`\\\\DESKTOP-davidol\\F\\Pic字典\\`。
